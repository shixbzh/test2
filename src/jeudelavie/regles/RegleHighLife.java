...
public class RegleHighLife implements Regles{

    private int[] conditions;

    public RegleHighLife(int[] conditions){
    ...
    public Etat etatFutur(Etat ancienEtat , int nbrCellV){
        ...
        if((nbrCellV < 2 || nbrCellV > 3 ) && ancienEtat == Etat.Vivant){
            return Etat.Vivant;
        }

        if((nbrCellV ==3 || nbrCellV ==6) && ancienEtat == Etat.Mort){
            return Etat.Vivant;
        }
        return Etat.Mort;
